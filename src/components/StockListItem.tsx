...
export default function StockListItem({ stock }: StockListItem) {
  const [runMutation] = useMutation(mutation, {
    variables: { user_id: "Mark", symbol: stock.symbol },
  });
    ...
    <Link href={`/${stock.symbol}`} asChild>
      <Pressable style={styles.container}>
        ...
        <View style={{ alignItems: "flex-end" }}>
          ...
          {/* If the number (change value) is positive, color it green, otherwise red */}
          <MonoText style={{ color: change > 0 ? "green" : "red" }}>
            {/* If the number is positive, add a plus sign, otherwise leave it negative */}
            {change > 0 ? "+" : ""}
            {change.toFixed(1)}%
          </MonoText>
        </View>
